...
Every release of `go-release-demo-goreleaser-svu` is signed by
...
This document shows how to verify a downloaded archive at three levels
of rigor.

Assumes you've downloaded these assets from a release page (e.g.
<https://github.com/bonial-oss/go-release-demo-goreleaser-svu/releases/tag/v0.1.0>):
...
```bash
cosign verify-blob \
  --certificate-identity-regexp='^https://github.com/bonial-oss/go-release-demo-goreleaser-svu/\.github/workflows/release\.yaml@refs/heads/main$' \
...
Level 1, that transitively verifies every archive listed in it.

## Level 3 — SLSA provenance (requires `slsa-verifier` v2.x)

Confirms the archive was built from a specific commit by a specific
...
```bash
git clone https://github.com/bonial-oss/go-release-demo-goreleaser-svu
cd go-release-demo-goreleaser-svu
git checkout v0.1.0
